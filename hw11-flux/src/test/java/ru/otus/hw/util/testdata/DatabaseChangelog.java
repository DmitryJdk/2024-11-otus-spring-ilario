...
public class DatabaseChangelog {
    ...
    public void insertAuthors(AuthorRepository authorRepository) {
        authorRepository.insert(TestDataUtil.authors).blockLast();
    }

    @ChangeSet(order = "003", id = "genres", author = "dmitryJdk", runAlways = true)
    public void insertGenres(GenreRepository genreRepository) {
        genreRepository.insert(TestDataUtil.genres).blockLast();
    }

    @ChangeSet(order = "004", id = "insertBooks", author = "dmitryJdk", runAlways = true)
    public void insertBooks(BookRepository repository) {
        repository.insert(TestDataUtil.books).blockLast();
    }
    ...
    public void insertComments(CommentRepository commentRepository) {
        commentRepository.insert(TestDataUtil.comments).blockLast();
    }
